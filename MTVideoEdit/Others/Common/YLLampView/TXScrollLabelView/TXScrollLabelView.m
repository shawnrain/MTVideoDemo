...
- (void)setupLRUDTypeLayoutWithTitle:(NSString *)title
                             maxSize:(CGSize)size
                              ...
                              height:(CGFloat)height
                    completedHandler:(void(^)(CGSize size))completedHandler {
    NSMutableAttributedString *attributed = (NSMutableAttributedString *)title;
    NSString *ll = [attributed.string copy];
    CGSize scrollLabelS = [ll boundingRectWithSize:size
                                              options:NSStringDrawingUsesLineFragmentOrigin
                                           attributes:@{NSFontAttributeName: self.font} context:nil].size;
    //回调获取布局数据
    completedHandler(scrollLabelS);
...
#pragma clang diagnostic ignored "-Wincompatible-pointer-types"
    setter(self, sel, self.upLabel.attributedText, TXScrollLabelTypeUp);
    setter(self, sel, self.downLabel.attributedText, TXScrollLabelTypeDown);
...
- (void)updateScrollText {
    ...
    self.upLabel.attributedText = self.scrollArray[currentSentence];
    currentSentence ++;
    if (currentSentence >= self.scrollArray.count) currentSentence = 0;
    self.downLabel.attributedText = self.scrollArray[currentSentence];
